...
describe('Test the creation of a user post', function() {
   it('Save a post and load it back', function(done) {
     ...
     var firstPost = Post.create("first@gmail.com", "Voxxed Days Belgrade 2016", "Voxxed Days Belgrade 2016 - seems really interesting", creationDate, 0);
     ...
     postRepository.save(firstPost, function(err, reply) {
        ...
        postRepository.getPost(postHashKey, function(actualPost) {
				...
				postRepository.getLastCreatedPosts(1, function(lastCreatedPostsList) {
					expect(lastCreatedPostsList[0].title).to.equal('Voxxed Days Belgrade 2016');
				...
				postRepository.getLastPostsPerUser(1, firstPost.emailAddress, function(postPerUserList) {
					expect(postPerUserList[0].title).to.equal('Voxxed Days Belgrade 2016');
					expect(postPerUserList[0].key).to.equal(postRepository.getPostsHashKey(firstPost.emailAddress, 'Voxxed Days Belgrade 2016'));
					allOperationsToDo--;
					areOperationsDone(allOperationsToDo, done);
				});

				postRepository.getTopVotedPosts(1, function(topVotedPostsList) {
					expect(topVotedPostsList[0].title).to.equal('Voxxed Days Belgrade 2016');
					expect(topVotedPostsList[0].votes).to.equal(0);
					expect(topVotedPostsList[0].key).to.equal(postRepository.getPostsHashKey(firstPost.emailAddress, 'Voxxed Days Belgrade 2016'));
					allOperationsToDo--;
					areOperationsDone(allOperationsToDo, done);
				});

				postRepository.getPosts(1, function(postsList) {
					expect(postsList[0].title).to.equal('Voxxed Days Belgrade 2016');
					expect(postsList[0].key).to.equal(postRepository.getPostsHashKey(firstPost.emailAddress, 'Voxxed Days Belgrade 2016'));
					allOperationsToDo--;
					areOperationsDone(allOperationsToDo, done);
				});
